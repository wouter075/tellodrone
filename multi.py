...
# -*- coding: utf-8 -*-
import sys
from tello.tello_manager import *
import queue
import time
import os
    ...
    for command in commands:
        if command != '' and command != '\n':
            ...
            if '//' in command:
                ...
                print(f'[IP_SN_FID]:Tello_IP:{sn_ip_dict[drone_sn]}------Tello_SN: {drone_sn}------Tello_fid:{drone_id}'
                      f'\n')
                # print id_sn_dict[drone_id]
            elif 'sync' in command:
                timeout = float(command.partition('sync')[2])
                print(f'[Sync_And_Waiting]Sync for {timeout} seconds \n')
                time.sleep(1)
                try:
                    start = time.time()
                    # wait till all commands are executed
